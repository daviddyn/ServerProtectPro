...
public class Path {
    ...
    public Path getInto(String[] patterns, int offset, int length) {
        checkEmptyPattern(patterns, offset, length);
        if (length == 0) {
            return this;
        }
    ...
    public boolean startsWith(Path prefix) {
        return startsWith(0, prefix.patterns, 0, prefix.patterns.length);
    }

    public boolean startsWith(int startOffset, Path prefix) {
        return startsWith(startOffset, prefix.patterns, 0, prefix.patterns.length);
    }

    public boolean startsWith(int startOffset, Path prefix, int startInclude, int endExclude) {
        return startsWith(startOffset, prefix.patterns, startInclude, endExclude - startInclude);
    }

    public boolean startsWith(String... patterns) {
        return startsWith(0, patterns, 0, patterns.length);
    }

    public boolean startsWith(int startOffset, String... patterns) {
        return startsWith(startOffset, patterns, 0, patterns.length);
    }

    public boolean startsWith(int startOffset, String[] patterns, int patternsOffset, int patternsLength) {
        if (startOffset > this.patterns.length || this.patterns.length - startOffset < patternsLength) {
            return false;
        }
        for (int i = 0; i < patternsLength; i++) {
            if (!Objects.equals(this.patterns[i + startOffset], patterns[i + patternsOffset])) {
                return false;
            }
        }
        return true;
    ...
    public boolean endsWith(Path suffix) {
        return endsWith(suffix.patterns, 0, suffix.patterns.length);
    }

    public boolean endsWith(Path suffix, int startInclude, int endExclude) {
        return endsWith(suffix.patterns, startInclude, endExclude - startInclude);
    }

    public boolean endsWith(String... patterns) {
        return endsWith(patterns, 0, patterns.length);
    }

    public boolean endsWith(String[] patterns, int patternsOffset, int patternsLength) {
        if (this.patterns.length < patternsLength) {
            return false;
        }
        return startsWith(this.patterns.length - patternsLength, patterns, patternsOffset,patternsLength);
    }
    ...
    public String toInverseString(String pathSeparator, StringEscapeEncoder encoder) {
        if (patterns.length == 0) {
            return "";
        }
        StringBuilder builder = new StringBuilder();
        for (int i = patterns.length; i > 0; i--) {
            if (encoder == null) {
                builder.append(patterns[i - 1]).append(pathSeparator);
            }
            else {
                builder.append(encoder.encode(patterns[i - 1])).append(pathSeparator);
            }
        }
        builder.delete(builder.length() - pathSeparator.length(), builder.length());
        return builder.toString();
    ...
    private static void checkEmptyPattern(String[] patterns, int offset, int length) {
        for (int i = 0; i < length; i++) {
            if (patterns[offset + i].isEmpty()) {
                throw new IllegalArgumentException("不允许空长的pattern");
            }
        }
    }
