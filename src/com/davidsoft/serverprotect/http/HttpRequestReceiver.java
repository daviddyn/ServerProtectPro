...
/**
 * 大致使用方法：
 * <p>1. 调用{@link #receive()}接收请求头, 根据返回的结果判断请求头语法是否正确、URL是否过长、请求头是否过大等。</p>
 * <p>2. 调用{@link #analyseContent()}分析请求头，分析出此请求是否包含Content，若包含，则继续分析Transfer-Encoding、Content-Encoding、Content-Length、Connection等字段是否合理。</p>
 * <p>4. 调用{@link #hasContent()}判断此请求是否包含Content。若不包含Content，则至此Http请求已接收完成。</p>
 * <p>5. 调用{@link #getContentInputStream()}获得接收Content的输入流。</p>
 * <p>6. 调用{@link #hasSuspendedHeaders()}判断此请求是否包含悬挂头。若不包含Content，则至此Http请求已接收完成。</p>
 * <p>7. 调用{@link #getSuspendedHeaders()}接收悬挂头。</p>
 */
public final class HttpRequestReceiver {
    ...
    /**
     * @param in 用于接收Http请求的输入流。
     * @param maxPathLength 最大URI字符数。
     * @param maxHeaderSize 最大请求头、悬挂头(分别)字节数。
     */
    public HttpRequestReceiver(InputStream in, int maxPathLength, int maxHeaderSize) {
     ...
     * <p></p>
     * <p>注意：此函数需要在成功调用{@link #receive()}函数后调用。</p>
     *
     ...
     */
    public int readSuspendedHeaders() throws IOException {
        if (!chunkedTransfer) {
            throw new IllegalStateException("当前的Http请求不包含悬挂头。");
        }
        suspendedHeaders = new HttpHeaders();
        return suspendedHeaders.fromRequestStreamLimited(in, new StringBuilder(), maxHeaderSize);
    }

    /**
     * <p>获得此Http请求的请求头(不包含悬挂头)。</p>
     * <p></p>
     * <p>注意：此函数需要在成功调用{@link #receive()}函数后调用。</p>
     */
     ...
     * <p>获得此Http请求的悬挂头。</p>
     * <p></p>
     * <p>注意：此函数需要在成功调用{@link #readSuspendedHeaders()}函数后调用。</p>
     */
